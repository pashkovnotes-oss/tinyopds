...
private Dictionary<string, int> CreateNavigationGroups(List<string> authors, string currentPattern, int threshold)
{
    ...
    {
        // только уникальные первые буквы, без подсчёта
        return authors
            .Select(a => a.Substring(0, 1).ToUpperInvariant())
            .Distinct()
    ...
    else
    {
        ...
        var prefixes = filteredAuthors
            .Where(a => a.Length > nextCharPos)
            .Where(a => char.IsLetter(a[nextCharPos]))
            .Select(a =>
            {
